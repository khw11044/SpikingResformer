...
class GWFFN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_feat_out = x.clone()      # ([4, 128, 64, 32, 32])
        x = self.up(x)              # ([4, 128, 64, 32, 32]) -> ([4, 128, 256, 32, 32])
        x_feat_in = x.clone()
        for m in self.conv:
            x = m(x)
        x = x + x_feat_in
...
class DSSA(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # X: [T, B, C, H, W]
        T, B, C, H, W = x.shape     # ([4, 128, 64, 32, 32])
        x_feat = x.clone()
        x = self.activation_in(x)   # ([4, 128, 64, 32, 32]) -> ([4, 128, 64, 32, 32])      # 0아님 1로 바뀜

        y = self.W(x)               # y: ([4, 128, 128, 8, 8])
        y = self.norm(y)            # y: ([4, 128, 128, 8, 8])
        y = y.reshape(T, B, self.num_heads, 2 * C // self.num_heads, -1)        # ([4, 128, 1, 128, 64])
        y1, y2 = y[:, :, :, :C // self.num_heads, :], y[:, :, :, C // self.num_heads:, :]
        x = x.reshape(T, B, self.num_heads, C // self.num_heads, -1)            # ([4, 128, 1, 64, 1024])
        ...
        out = self.matmul2(y2, attn)        # ([4, 128, 1, 64, 64]) * ([4, 128, 1, 64, 1024]) = ([4, 128, 1, 64, 1024])
        out = out * scale2
        out = out.reshape(T, B, C, H, W)    # ([4, 128, 64, 32, 32])
        out = self.activation_out(out)      # ([4, 128, 64, 32, 32])

        out = self.Wproj(out)               # Conv1x1
        out = self.norm_proj(out)           # BN
        out = out + x_feat                  # ([4, 128, 64, 32, 32]) + ([4, 128, 64, 32, 32])
        return out
...
class SpikingResformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() != 5:
            ...
            #### [B, T, C, H, W] -> [T, B, C, H, W]
            x = x.transpose(0, 1)
        x = self.prologue(x)                # ([4, 128, 3, 32, 32]) -> ([4, 128, 64, 32, 32])
        x = self.layers(x)                  # ([4, 128, 64, 32, 32]) -> ([4, 128, 384, 8, 8]) # DSSA 와 GWFFN로 이루어진 레이어
        x = self.avgpool(x)                 # ([4, 128, 384, 8, 8]) -> ([4, 128, 384, 1, 1])
        x = torch.flatten(x, 2)             # ([4, 128, 384, 1, 1]) -> ([4, 128, 384])
        x = self.classifier(x)              # ([4, 128, 384]) -> ([4, 128, 10])     # 10개의 클래스 별
        return x
