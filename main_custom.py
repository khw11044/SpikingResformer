...
def parse_args():
    ...
    config_parser.add_argument(
        ...
        default='configs/direct_training/cifar10.yaml'
    )
    ...
    parser.add_argument('--batch-size', default=1024, type=int)       # 32
    ...
    parser.add_argument('--workers', default=8, type=int, help='number of data loading workers')       # 16
    ...
    parser.add_argument('--output-dir', default='outputs/logs/temp')      # './logs/temp'
...
def train_one_epoch(
    ...
    model.zero_grad()
    for idx, (image, target) in tqdm(enumerate(data_loader_train), total=len(data_loader_train)):
        with GlobalTimer('iter', timer_container):
            image, target = image.cuda(), target.cuda()
            if one_hot:
                target = F.one_hot(target, one_hot).float()
            if scaler is not None:
                # 모델 훈련 속도와 GPU 메모리 효율성을 크게 개선할 수 있기 때문에 혼합 정밀도 훈련을 사용하는 것이 좋음, 특히 대규모 딥러닝 모델에서는 훈련 시간이 줄고, 더 큰 배치 크기를 사용할 수 있어 훈련 성능을 최적화할 수 있음
                with autocast('cuda'):                        # PyTorch에서 자동 혼합 정밀도(Automatic Mixed Precision, AMP)를 사용할 때 GPU 장치를 지정하는 방식
                    output = model(image)
